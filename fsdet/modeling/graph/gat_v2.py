...
class GATv2Conv(MessagePassing):
    ...
    def __init__(self, in_channels: int,
        ...
        self.att = nn.Parameter(torch.Tensor(1, heads, out_channels))  # (1, 1, 128)

        if bias and concat:
            self.bias = nn.Parameter(torch.Tensor(heads * out_channels))  # 128
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
         ...
         # x: (1024, 1024), edge_index: (2, 1024*1024) [[0,0],[0,1]....] 节点
        r"""
        ...
        x_l: OptTensor = None  # (1024, 1, 128)
        x_r: OptTensor = None  # (1024, 1, 128)
...
class GAT(BasicGNN):
    ...
    def init_conv(self, in_channels: int, out_channels: int,
        ...
        Conv2d = GATConv if not v2 else GATv2Conv

        return Conv2d(in_channels, out_channels, heads=heads, concat=concat,
