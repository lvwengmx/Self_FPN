...
class FastRCNNContrastOutputs(FastRCNNOutputs):
    ...
    def __init__(
        ...
        box_type = type(proposals[0].proposal_boxes) # (256, 4)
        # cat(..., dim=0) concatenates over all images in the batch
        self.proposals = box_type.cat([p.proposal_boxes for p in proposals])  # (1024, 4) self.proposals = List[Boxes]
        assert not self.proposals.tensor.requires_grad, "Proposals should not require gradients!"

        # 每个 proposals 包括 [num_instances=256, image_height, image_width, fields=[proposal_boxes=(256, 4), objectness_logits=256, gt_class=256, iou=256, gt_boxes=(256, 4)]]
        self.image_shapes = [x.image_size for x in proposals]

        # The following fields should exist only when training.
        if proposals[0].has("gt_boxes"):
            self.gt_boxes = box_type.cat([p.gt_boxes for p in proposals]) #  gt_boxes=(1024, 4)
            assert proposals[0].has("gt_classes")
            self.gt_classes = cat([p.gt_classes for p in proposals], dim=0)  # gt_classes=1024
            self.ious = cat([p.iou for p in proposals], dim=0) # iou=1024
...
class CosineSimOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        nn.init.normal_(self.cls_score.weight, std=0.01)  #(1024, 20+1)
        nn.init.normal_(self.bbox_pred.weight, std=0.001)  # (1024, 80)
    ...
    def forward(self, x):
        ...
        scores = self.scale * cos_dist   # (1024, 21)
        proposal_deltas = self.bbox_pred(x) # (1024, 80)
        return scores, proposal_deltas
