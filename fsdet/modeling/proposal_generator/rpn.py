...
class RPN(nn.Module):
    ...
    def forward(self, images, features, features_crop1, features_crop2, gt_instances=None):
        """
        Args:
        ...
        if self.training:
            features_crop1 = [self.max_pool(features_crop1[f]) for f in self.in_features]
            features_crop2 = [self.max_pool(features_crop2[f]) for f in self.in_features]
