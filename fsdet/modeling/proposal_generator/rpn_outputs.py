...
def rpn_losses(
    ...
    for tensor1, tensor2 in zip(P_1, P_2):
        similarity = chamfer_distance(tensor1, tensor2)
        # similarity = MSE_loss(tensor1, tensor2)
        similarity_list.append(similarity)
    Similarity_sum = torch.zeros_like(similarity_list[0])
    for tensor in similarity_list:
        Similarity_sum = torch.add(Similarity_sum, tensor)
    return objectness_loss, localization_loss, Similarity_sum
...
class RPNOutputs(object):
    def __init__(
        ...
        Args:
            ...
            smooth_l1_beta (float): The transition point between L1 and L2 loss in
                ...
                set to +inf, the loss becomes constant 0.
        """
        self.p_1 = features_crop1
        self.p_2 = features_crop2
    ...
    def losses(self):
        ...
        objectness_loss, localization_loss, similarity_sum_loss = rpn_losses(
            gt_objectness_logits,
            gt_anchor_deltas,
            pred_objectness_logits,
            pred_anchor_deltas,
        ...
        loss_sim = similarity_sum_loss * normalizer # sim: feature similarity loss
        losses = {"loss_rpn_cls": loss_cls, "loss_rpn_loc": loss_loc, "loss_rpn_sim": loss_sim}
