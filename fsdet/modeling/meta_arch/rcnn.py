...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        features_crop1 = self.backbone(images_crop_1.tensor)
        features_crop2 = self.backbone(images_crop_2.tensor)
        ...
        # if self.proposal_generator:
            proposals, proposal_losses = self.proposal_generator(images, features, features_crop1, features_crop2, gt_instances)
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if detected_instances is None:
            if self.proposal_generator:
                proposals, _ = self.proposal_generator(images, features, features_crop1, features_crop2, None)
            else:
                assert "proposals" in batched_inputs[0]
                proposals = [x["proposals"].to(self.device) for x in batched_inputs]
    ...
    def preprocess_image(self, batched_inputs):
        ...
        images_crop_1 = [self.croped_image(x) for x in images]
        images_crop_2 = [self.croped_image(x) for x in images]
        images = ImageList.from_tensors(images, self.backbone.size_divisibility)
        images_crop_1 = ImageList.from_tensors(images_crop_1, self.backbone.size_divisibility)
        images_crop_2 = ImageList.from_tensors(images_crop_2, self.backbone.size_divisibility)
        return images, images_crop_1, images_crop_2
