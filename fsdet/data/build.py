...
# from .loader_transform import TwoCropsTransform
...
def get_detection_dataset_dicts(dataset_names, filter_empty=True):
    ...
    for dataset_name, dicts in zip(dataset_names, dataset_dicts):
        ...
        # '---novel classes---'
    #     for dict_i in dicts:
    #         index = dict_i['annotations'][0]['category_id']
    #         keep = index < 15
    #         if keep is True:
    #            dict_i.clear()
    # while {} in dicts:
    #     dicts.remove({})
    #dataset_dicts = select_dataset
    ...
    if filter_empty and has_instances:
        ...
        for i in range(len(dataset_dicts)):
            label = dataset_dicts[i]['annotations'][:1][0]['category_id']
            if label not in select_count or select_count[label] < 2:
                select_indices.append(i)
                if label not in select_count:
                    select_count[label] = 1
                else:
                    select_count[label] += 1
            if len(select_count) == 10 and all(count == 10 for count in select_count.values()):
                break

        select_dataset = [dataset_dicts[i] for i in select_indices]
        dataset_dicts = select_dataset
...
#         normalize
#     ]
#     img_tansfomer = Image.open(x)
#     # x_transformer = transforms.Compose(augmentation)(img_tansfomer)
#     x_transformer = TwoCropsTransform(transforms.Compose(augmentation))(img_tansfomer)
#
